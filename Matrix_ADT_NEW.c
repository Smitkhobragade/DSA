...
void display_row_major(struct matrix a){
	printf("\n ROW MAJOR FORM OF THE ARRAY MATRIX IS : ");
	printf("\n");
    for(int i = 0;i<a.lastrow_index;i++){
        for(int j = 0; j<a.lastcol_index;j++){
            printf("%d\t",a.a[i][j]);
...
void display_col_major(struct matrix a){
	printf("\n COLUMN MAJOR FORM OF THE ARRAY MATRIX IS : ");
	printf("\n");
    for(int i = 0;i<a.lastrow_index;i++){
        for(int j = 0; j<a.lastcol_index;j++){
            printf("%d\t",a.a[j][i]);
...
int main(){
    ...
    display(&result);
	display_row_major(arr1);
	display_col_major(arr1);
